...
def nosso_numero():
    ...
    with open(gf.ret_file_path, 'r') as ret_file1:
        for i in ret_file1.readlines()[
                 1:-1]:  # OBSERVE O [1:] -> Indica que iremos ignorar o cabeçalho na hora da leitura
            element = i[70:82]
            ls_nosso_numero.append(element)

    df_nosso_numero = pd.DataFrame(ls_nosso_numero, columns=['nosso_numero'])
    # df_nosso_numero = df_nosso_numero.apply(pd.to_numeric, errors='coerce')

# def seu_numero():
#     global df_seu_numero
#
#     ls_seu_numero = []
#     with open(gf.ret_file_path, 'r') as ret_file1:
#         for i in ret_file1.readlines()[1:-1]:  # OBSERVE O [1:] -> Indica que iremos ignorar o cabeçalho na hora da leitura
#             element = i[42:52]
#             ls_seu_numero.append(element)
#
#     df_seu_numero = pd.DataFrame(ls_seu_numero, columns=['seu_numero'])
#     # df_nosso_numero = df_nosso_numero.apply(pd.to_numeric, errors='coerce')
...
def dt_vencimento():
    ...
    df_dt_vencimento = pd.DataFrame(ls_dt_vencimento, columns=['dt_vencimento'])
...
def dt_ocorrencia():
    ...
    df_dt_ocorrencia = pd.DataFrame(ls_dt_ocorrencia, columns=['dt_ocorrencia'])
...
def aviso_bancario():
    global aviso_bancario

    aviso_bancario = ''
    with open(gf.ret_file_path, 'r') as ret_file1:
        for i in ret_file1.readlines()[0:1]:
            aviso_bancario = i[108:113]
...
#essa função depende da fille_fitter() para fucionar, fille_fitter() deve ser invocada antes
